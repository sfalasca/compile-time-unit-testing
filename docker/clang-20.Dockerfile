...
# Compiler matrix: Clang 20 (silkeh/clang, Debian)
#
# Standalone image: `docker build -f docker/clang-20.Dockerfile -t ctut-clang20 .`
...
RUN apt-get update && apt-get install -y --no-install-recommends python3-pip ninja-build libc++-20-dev libc++abi-20-dev \
