...
# Compiler matrix: Clang 13 (silkeh/clang, Debian)
#
# Standalone image: `docker build -f docker/clang-13.Dockerfile -t ctut-clang13 .`
...
RUN apt-get update && apt-get install -y --no-install-recommends python3-pip ninja-build libc++-13-dev libc++abi-13-dev \
