...
# Compiler matrix: Clang 19 (silkeh/clang, Debian)
#
# Standalone image: `docker build -f docker/clang-19.Dockerfile -t ctut-clang19 .`
...
RUN apt-get update && apt-get install -y --no-install-recommends python3-pip ninja-build libc++-19-dev libc++abi-19-dev \
