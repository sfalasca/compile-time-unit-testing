...
# Compiler matrix: Clang 14 (silkeh/clang, Debian)
#
# Standalone image: `docker build -f docker/clang-14.Dockerfile -t ctut-clang14 .`
...
RUN apt-get update && apt-get install -y --no-install-recommends python3-pip ninja-build libc++-14-dev libc++abi-14-dev \
