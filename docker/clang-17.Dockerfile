...
# Compiler matrix: Clang 17 (silkeh/clang, Debian)
#
# Standalone image: `docker build -f docker/clang-17.Dockerfile -t ctut-clang17 .`
...
RUN apt-get update && apt-get install -y --no-install-recommends python3-pip ninja-build libc++-17-dev libc++abi-17-dev \
