...
# Compiler matrix: Clang 18 (silkeh/clang, Debian)
#
# Standalone image: `docker build -f docker/clang-18.Dockerfile -t ctut-clang18 .`
...
RUN apt-get update && apt-get install -y --no-install-recommends python3-pip ninja-build libc++-18-dev libc++abi-18-dev \
